class Revenue < ApplicationRecord
  ...
  validates :month, presence: true, numericality: { greater_than_or_equal_to: 1, less_than_or_equal_to: 12, only_integer: true }
  ...
  def self.total_revenue_for_year(year)
    where(year: year).group(:payment_type).sum(:amount)
  end

  def self.monthly_revenue_for_year_and_type(year, type)
    where(year: year, payment_type: type).order(:month).pluck(:amount)
  end
