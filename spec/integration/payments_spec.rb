...
RSpec.describe 'Api::V1::PaymentsController', type: :request do
  ...
  path '/api/v1/payments/{id}/apply' do
    post 'Apply a payment' do
      tags 'Payments'
      security [bearerAuth: []]
      consumes 'application/json'
      produces 'application/json'
      ...
      response '200', 'Payment applied successfully' do
        let(:id) { payment.id }
        let(:payment_params) do
          {
            payment: {
              amount: payment.amount,
              interest_amount: payment.interest_amount || 0.0,
              total_amount: payment.amount + (payment.interest_amount || 0.0)
            }
          }
        end

        before do
          # Ensure contract is approved so pay! can update balance and close contract if needed
        ...
        run_test! do |response|
          ...
          expect(json['message']).to match(/applied/i)

          payment.reload
          expect(payment.status).to eq('paid')
          contract.reload
          expect(%w[approved closed]).to include(contract.status)
        end
      end
    end
  end
